(function(){
  ...
  // Central de Encomendas: V942 — Em aberto global + detalhes expandem o próprio card.
  var fsTarget=q('.enc-page');
  var activeStatus='nova';
  var viewMode='status';
  var periodMode='next7';
  var rangeFrom='';
  var rangeTo='';
  ...
  function getCurrentRange(){
    if(periodMode==='today'){var t=todayYmd();return {from:t,to:t}}
    if(periodMode==='next7'){var from=addDaysYmd(todayYmd(),1);return {from:from,to:addDaysYmd(todayYmd(),7)}}
    if(periodMode==='range')return normalizeRange(rangeFrom,rangeTo)
    var fallback=todayYmd();return {from:fallback,to:fallback}
  }
  ...
  function syncPeriodUi(){
    ...
    qa('[data-enc-period]',fsTarget).forEach(function(btn){btn.classList.toggle('active',(btn.dataset.encPeriod||'')===periodMode)});
    var range=getCurrentRange();
    if(periodMode==='today'||periodMode==='next7'){rangeFrom='';rangeTo='';if(inputFrom)inputFrom.value=range.from;if(inputTo)inputTo.value=range.to}
    else if(periodMode==='range'){if(inputFrom)inputFrom.value=rangeFrom;if(inputTo)inputTo.value=rangeTo}
    var label=q('[data-enc-period-metric-label]',fsTarget),sub=q('[data-enc-period-metric-subtitle]',fsTarget);
    if(label)label.textContent=periodMode==='today'?'Para hoje':'No período';
    if(sub)sub.textContent=periodMode==='today'?'Retiradas e entregas do dia':'Encomendas ativas no intervalo';
  }
  function statusCountsForPeriod(){
    ...
    allOrderCards().forEach(function(card){
      ...
      /* V941: Novas é caixa de entrada global; os demais status seguem o período. */
      if(status==='nova'||cardMatchesDate(card))counts[status]+=1;
    });
    return counts;
  }
  ...
  function cardVisibleForMode(card){
    ...
    /* V941: Novas permanece visível até aceitar/recusar, independente da data. */
    if(activeStatus==='nova')return status==='nova';
  ...
  function emptyCopy(){
    ...
    return ['Nenhuma encomenda '+(labels[activeStatus]||''),'Não há encomendas deste status no período selecionado.'];
  }
  ...
  function cardsForPrint(scope){
    ...
    if(scope.indexOf('status:')===0){var wanted=scope.split(':')[1]||'';return cards.filter(function(card){return cardStatus(card)===wanted&&(wanted==='nova'||cardMatchesDate(card))})}
  ...
  function printScopeLabel(scope){
    ...
    var range=getCurrentRange(),period='Período: '+formatDatePtBr(range.from)+' até '+formatDatePtBr(range.to);
    if(scope==='period')return period+' · encomendas ativas';if(scope==='receivable')return period+' · saldos a receber';
    if(scope.indexOf('status:')===0){var key=scope.split(':')[1],labels={nova:'Novas',confirmada:'Confirmadas',producao:'Em produção',pronta:'Prontas',finalizada:'Finalizadas',cancelada:'Canceladas'};if(key==='nova')return 'Novas · todas as datas';return period+' · '+(labels[key]||key)}
    return period;
  }
  ...
  var detailBackdropV943=q('#encDetailBackdropV943');
  function detailText(root,selector){var el=q(selector,root);return String((el&&el.textContent)||'').replace(/\s+/g,' ').trim()}
  function detailSet(id,value){var el=q('#'+id);if(el)el.textContent=String(value||'—')}
  function cleanDetailLabel(text){return String(text||'').replace(/^Personalização:\s*/i,'').replace(/\s+/g,' ').trim()}
  function closeOrderDetailsV943(){
    if(!detailBackdropV943)return;
    detailBackdropV943.classList.remove('open');
    detailBackdropV943.setAttribute('aria-hidden','true');
    document.body.style.overflow='';
    setTimeout(function(){if(!detailBackdropV943.classList.contains('open'))detailBackdropV943.hidden=true},170);
  }
  function openOrderDetailsV943(button){
    var card=button&&button.closest('[data-enc-order]');if(!card||!detailBackdropV943)return;
    var number=card.dataset.encNumber||'',client=card.dataset.encClient||'';
    detailSet('encDetailTitleV943',(number?'#'+number:'Encomenda')+(client?' · '+client:''));
    var status=detailText(card,'[data-enc-status-trigger]')||detailText(card,'.enc-status')||card.dataset.encStatus||'—';
    detailSet('encDetailStatusV943',status);
    detailSet('encDetailPhoneV943',card.dataset.encPhone||'Não informado');
    detailSet('encDetailKindV943',card.dataset.encKind||detailText(card,'.enc-fulfillment-purpose strong')||'—');
    var date=detailText(card,'.enc-fulfillment-date strong')||card.dataset.encDate||'';
    var time=detailText(card,'.enc-fulfillment-time strong')||card.dataset.encTime||'';
    detailSet('encDetailDateTimeV943',[date,time].filter(Boolean).join(' · ')||'—');
    var address=String(card.dataset.encAddress||'').trim(),addressBox=q('#encDetailAddressBoxV943');
    if(addressBox){addressBox.hidden=!address;if(address)detailSet('encDetailAddressV943',address)}
    var itemsBox=q('#encDetailItemsV943');
    if(itemsBox){
      itemsBox.replaceChildren();
      var lines=qa('.enc-product-line',card);
      if(!lines.length){var empty=document.createElement('div');empty.className='enc-detail-text-v943';empty.textContent='Nenhum item informado.';itemsBox.appendChild(empty)}
      lines.forEach(function(line){
        var row=document.createElement('div');row.className='enc-detail-item-v943';
        var strong=document.createElement('strong');strong.textContent=detailText(line,'strong')||'Item';
        var span=document.createElement('span');span.textContent=detailText(line,'span')||'';
        row.appendChild(strong);row.appendChild(span);itemsBox.appendChild(row);
      });
    }
    var obs=cleanDetailLabel(detailText(card,'.enc-personalization')),obsBox=q('#encDetailObsBoxV943');
    if(obsBox){obsBox.hidden=!obs;if(obs)detailSet('encDetailObsV943',obs)}
    detailSet('encDetailTotalV943',detailText(card,'.enc-payment > div strong')||'—');
    var chip=detailText(card,'.enc-finance-chip'),finance=detailText(card,'.enc-finance-detail');
    detailSet('encDetailPaymentV943',[chip,finance].filter(Boolean).join(' · ')||'Não informado');
    detailBackdropV943.hidden=false;detailBackdropV943.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';
    requestAnimationFrame(function(){detailBackdropV943.classList.add('open')});
  }
  ...
  if(fsTarget){
    ...
    fsTarget.addEventListener('click',function(e){
      ...
      var detail=e.target.closest('[data-enc-detail]');if(detail){e.preventDefault();e.stopPropagation();openOrderDetailsV943(detail);return}
    ...
    fsTarget.addEventListener('change',function(e){if(e.target&&(e.target.id==='encDateFrom'||e.target.id==='encDateTo')){var from=dateInputFrom(),to=dateInputTo();rangeFrom=from?String(from.value||''):'';rangeTo=to?String(to.value||''):'';if(rangeFrom&&rangeTo&&rangeFrom>rangeTo){var tmp=rangeFrom;rangeFrom=rangeTo;rangeTo=tmp}if(!rangeFrom&&!rangeTo){periodMode='next7'}else{periodMode='range'}syncPeriodUi();applyCentralView();scrollToCurrentView()}});
  ...
  qa('[data-enc-detail-close-v943]').forEach(function(btn){btn.addEventListener('click',closeOrderDetailsV943)});
  if(detailBackdropV943)detailBackdropV943.addEventListener('click',function(e){if(e.target===detailBackdropV943)closeOrderDetailsV943()});
  document.addEventListener('keydown',function(e){if(e.key==='Escape'&&detailBackdropV943&&!detailBackdropV943.hidden)closeOrderDetailsV943()});
