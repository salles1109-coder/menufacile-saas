/* MenuFacile V964 — corrige navegação de mês sem fechar o calendário. */
(function(){
  'use strict';
  /* MenuFacile V961 — calendário público futuro livre, sem limite máximo. */
  ...
  function setupDate(nativeId,buttonId,popId){
    ...
    /* V961 — calendário público: futuro sem limite máximo.
       Se qualquer script antigo tentar recolocar max, removemos novamente. */
    if(nativeId==='encModalDate'){
      input.removeAttribute('max');
      new MutationObserver(function(){if(input.hasAttribute('max'))input.removeAttribute('max')}).observe(input,{attributes:true,attributeFilter:['max']});
    }
    floatPopover(pop);
    ...
    function within(v){
      if(input.min&&v<input.min)return false;
      if(nativeId==='encModalDate')return true;
      if(input.max&&v>input.max)return false;
      return true;
    }
    function normalizeView(){
      var selected=parseYmd(input.value),min=parseYmd(input.min),max=(nativeId==='encModalDate'?null:parseYmd(input.max));
    ...
    function monthAllowed(year,month){
      var first=new Date(year,month,1,12),last=new Date(year,month+1,0,12),min=parseYmd(input.min),max=(nativeId==='encModalDate'?null:parseYmd(input.max));
    ...
    function render(){
      ...
      if(nativeId==='encModalDate'&&input.hasAttribute('max'))input.removeAttribute('max');
      var selected=parseYmd(input.value),today=new Date(),min=parseYmd(input.min),max=(nativeId==='encModalDate'?null:parseYmd(input.max));
      ...
      var html='<div class="mf-picker-calendar-head"><button type="button" class="mf-picker-nav" data-mf-cal-prev aria-label="Mês anterior"'+(monthAllowed(prevYear,prevMonth)?'':' disabled')+'><i class="fa-solid fa-chevron-left"></i></button><div class="mf-picker-month">'+monthNames[month]+' de '+year+'</div><button type="button" class="mf-picker-nav" data-mf-cal-next aria-label="Próximo mês"'+((nativeId==='encModalDate'||monthAllowed(nextYear,nextMonth))?'':' disabled')+'><i class="fa-solid fa-chevron-right"></i></button></div>';
    ...
    pop.addEventListener('click',function(e){
      /* V964 — o calendário redesenha os botões de navegação com innerHTML.
         Sem parar a propagação, o clique chega ao document depois que o botão
         antigo já saiu do DOM; o handler global interpreta como clique fora e
