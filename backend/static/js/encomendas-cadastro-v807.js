(function(){
  ...
  // v807: criar categoria nova é um fluxo separado da edição e nunca recarrega a tela.
  const newCategoryForm=q('.enc-new-category-form');
  function categoryBaseUrl(){
    return String(newCategoryForm?.action||'').replace(/\/$/,'');
  }
  ...
  function createCategoryModalRow(data){
    ...
    row.innerHTML=`
      <form method="post" action="${categoryBaseUrl()}/${encodeURIComponent(id)}/editar" class="enc-category-edit-form" data-enc-category-id="${id}">
        ...
        <input name="nome" class="enc-category-name-input" required aria-label="Nome da categoria">
  ...
  newCategoryForm?.addEventListener('submit',async e=>{
    ...
    try{
      const res=await fetch(newCategoryForm.action,{method:'POST',credentials:'same-origin',headers:{'X-Requested-With':'XMLHttpRequest','Accept':'application/json'},body:new FormData(newCategoryForm)});
      const data=await res.json().catch(()=>({}));
      if(!res.ok||!data.ok)throw new Error(data.erro||'Não foi possível criar a categoria.');
      syncCreatedCategory(data);
      if(input)input.value='';
      if(data.existente)showToast('Essa categoria já existe. Ela foi selecionada.');
      else showToast('Nova categoria adicionada.');
  ...
  document.addEventListener('submit',async e=>{
    ...
    if(editCategoryForm){
      e.preventDefault();
      const input=q('input[name="nome"]',editCategoryForm);
      const name=String(input?.value||'').trim();
      if(!name){input?.focus();return;}
