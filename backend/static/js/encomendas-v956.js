(function(){
  ...
  function syncClearDateVisibilityV956(){
    if(!fsTarget)return;
  ...
  function syncPeriodUi(){
    ...
    var clear=q('[data-enc-clear-dates-v948]',fsTarget);if(clear)clear.hidden=periodMode==='none';syncClearDateVisibilityV956();
  }
  ...
  if(fsTarget){
    readActiveStatus();activeStatus='nova';viewMode='status';syncPeriodUi();syncClearDateVisibilityV956();applyCentralView();syncFullscreenButton();window.addEventListener('pageshow',syncClearDateVisibilityV956);setTimeout(syncClearDateVisibilityV956,0);
    fsTarget.addEventListener('click',function(e){
      var clearDates=e.target.closest('[data-enc-clear-dates-v948]');if(clearDates){e.preventDefault();rangeFrom='';rangeTo='';periodMode='none';syncPeriodUi();syncClearDateVisibilityV956();applyCentralView();return}
    ...
    fsTarget.addEventListener('change',function(e){if(e.target&&(e.target.id==='encDateFrom'||e.target.id==='encDateTo')){var from=dateInputFrom(),to=dateInputTo();rangeFrom=from?String(from.value||''):'';rangeTo=to?String(to.value||''):'';if(rangeFrom&&rangeTo&&rangeFrom>rangeTo){var tmp=rangeFrom;rangeFrom=rangeTo;rangeTo=tmp}periodMode=(!rangeFrom&&!rangeTo)?'none':'range';syncPeriodUi();syncClearDateVisibilityV956();applyCentralView();scrollToCurrentView()}});
