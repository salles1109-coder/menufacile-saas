(function(){
  ...
  qa('[data-enc-category-filter]').forEach(btn=>btn.addEventListener('click',()=>{
  ...
  // v805: editar categoria diretamente pelo card, abrindo o gerenciador no ponto certo.
  qa('[data-enc-edit-category]').forEach(btn=>btn.addEventListener('click',()=>{
    const id=String(btn.dataset.encEditCategory||'');
    openCategories();
    setTimeout(()=>{
      const row=q(`[data-enc-category-row="${id}"]`,categoryModal||document);
