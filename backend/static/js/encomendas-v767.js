(function(){
  ...
  function getCurrentRange(){
    ...
    return normalizeRange(inputFrom&&inputFrom.value,inputTo&&inputTo.value)
  }
  ...
  function syncPeriodUi(){
    if(!fsTarget)return;
    var inputFrom=dateInputFrom(),inputTo=dateInputTo();
    qa('[data-enc-period]',fsTarget).forEach(function(btn){var on=(btn.dataset.encPeriod||'')===periodMode;btn.classList.toggle('active',on)});
    var range=getCurrentRange();
    if(periodMode==='today'&&inputFrom&&inputTo){inputFrom.value=range.from;inputTo.value=range.to}
    if(periodMode==='next7'&&inputFrom&&inputTo){inputFrom.value=range.from;inputTo.value=range.to}
    if(periodMode==='range'&&inputFrom&&inputTo&&inputFrom.value&&inputTo.value&&inputFrom.value>inputTo.value){var tmp=inputFrom.value;inputFrom.value=inputTo.value;inputTo.value=tmp}
  }
  function refreshStatusCounts(){
    if(!fsTarget)return;
    var counts={nova:0,confirmada:0,producao:0,pronta:0,finalizada:0,cancelada:0};
    qa('[data-enc-order]',fsTarget).forEach(function(card){var status=(card.dataset.encStatusCard||card.dataset.encStatus||'').trim();if(!status||!counts.hasOwnProperty(status))return;if(cardMatchesDate(card))counts[status]+=1});
    qa('[data-enc-status-filter]',fsTarget).forEach(function(btn){var key=(btn.dataset.encStatusFilter||'').trim();var badge=q('.enc-status-number',btn);if(badge&&counts.hasOwnProperty(key))badge.textContent=String(counts[key])});
  }
  function applyCentralStatus(){
    ...
    qa('[data-enc-status-panel]',fsTarget).forEach(function(panel){
      ...
      if(kind==='fluxo'){
        ...
        var title=q('[data-enc-flow-title]',panel),subtitle=q('[data-enc-flow-subtitle]',panel);
        if(title)title.textContent=copy[0];if(subtitle)subtitle.textContent=copy[1];
  ...
  async function refreshCentralInPlace(url,targetStatus){
    ...
    syncPeriodUi();applyCentralStatus();syncFullscreenButton();scrollToActivePanel();
  }
  ...
  function collectPrintableOrders(){
    var cards=qa('[data-enc-order]',fsTarget).filter(function(card){return cardMatchesDate(card)});
    cards.sort(function(a,b){var ad=(a.dataset.encDate||'')+' '+(a.dataset.encTime||''),bd=(b.dataset.encDate||'')+' '+(b.dataset.encTime||'');return ad.localeCompare(bd)});
    return cards.map(function(card){
      var items=qa('.enc-product-line',card).map(function(line){return '<li><strong>'+line.querySelector('strong').textContent+'</strong> <span>'+((line.querySelector('span')||{}).textContent||'')+'</span></li>'}).join('');
      var obs=(q('.enc-personalization',card)||{}).textContent||'';
      var total=((q('.enc-payment > div strong',card)||{}).textContent)||'';
      var chip=((q('.enc-finance-chip',card)||{}).textContent||'').trim();
      var detail=((q('.enc-finance-detail',card)||{}).textContent||'').trim();
      var status=((q('.enc-status',card)||{}).textContent||'').trim();
      return {
        numero:card.dataset.encNumber||'',cliente:card.dataset.encClient||'',telefone:card.dataset.encPhone||'',tipo:card.dataset.encKind||'',data:card.dataset.encDate||'',hora:card.dataset.encTime||'',endereco:card.dataset.encAddress||'',items:items,obs:obs,total:total,pagamento:[chip,detail].filter(Boolean).join(' · '),status:status
      }
    });
  }
  function printCurrentOrders(){
    var orders=collectPrintableOrders();
    if(!orders.length){toast('Não há encomendas no período selecionado.');return}
    var company=(fsTarget.dataset.encCompany||'Empresa').replace(/[&<>\"]/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]});
    var range=getCurrentRange();
    var periodLabel='Todas as encomendas';
    if(range.from&&range.to)periodLabel='Período: '+formatDatePtBr(range.from)+' até '+formatDatePtBr(range.to);
    else if(range.from)periodLabel='A partir de '+formatDatePtBr(range.from);
    else if(range.to)periodLabel='Até '+formatDatePtBr(range.to);
    var body=orders.map(function(o){
      return '<section class="order">'
        +'<div class="head"><div><h3>#'+o.numero+' · '+o.cliente+'</h3><div class="sub">'+(o.telefone||'')+' · '+(o.tipo||'')+' · '+formatDatePtBr(o.data)+' · '+(o.hora||'')+'</div></div><div class="status">'+(o.status||'')+'</div></div>'
        +(o.endereco&&String(o.tipo).toLowerCase().indexOf('entrega')!==-1?'<div class="box"><strong>Endereço:</strong> '+o.endereco+'</div>':'')
        +'<div class="items"><ul>'+o.items+'</ul></div>'
        +(o.obs?'<div class="box"><strong>Observação:</strong> '+o.obs+'</div>':'')
        +'<div class="foot"><div><strong>Total:</strong> '+o.total+'</div><div><strong>Pagamento:</strong> '+o.pagamento+'</div></div>'
        +'</section>'
    }).join('');
    var html='<!doctype html><html><head><meta charset="utf-8"><title>Encomendas</title><style>body{font-family:Arial,Helvetica,sans-serif;color:#1f2937;margin:24px}h1{margin:0 0 4px;font-size:22px}p.meta{margin:0 0 18px;color:#667085;font-size:12px}.order{border:1px solid #d8dee6;border-radius:12px;padding:14px 16px;margin:0 0 14px;page-break-inside:avoid}.head{display:flex;justify-content:space-between;gap:12px;align-items:flex-start;margin-bottom:10px}.head h3{margin:0 0 4px;font-size:18px}.sub{font-size:12px;color:#667085}.status{font-size:12px;font-weight:700;border:1px solid #d8dee6;border-radius:999px;padding:6px 10px}.items ul{margin:0;padding-left:18px}.items li{margin:0 0 5px}.items span{color:#667085}.box{margin-top:10px;padding:9px 10px;border-radius:10px;background:#f8fafc;font-size:12px}.foot{margin-top:12px;display:flex;justify-content:space-between;gap:12px;font-size:13px}@media print{body{margin:12px}.order{break-inside:avoid}}</style></head><body><h1>'+company+' — Encomendas</h1><p class="meta">'+periodLabel+'</p>'+body+'<script>window.onload=function(){window.print();setTimeout(function(){window.close();},180);}<\/script></body></html>';
  ...
  if(fsTarget){
    readActiveStatus(); syncPeriodUi(); applyCentralStatus(); syncFullscreenButton();
    fsTarget.addEventListener('click',function(e){
      ...
      var period=e.target.closest('[data-enc-period]');
      if(period){var kind=period.dataset.encPeriod||''; if(kind==='today'){periodMode='today';syncPeriodUi();applyCentralStatus();return} if(kind==='next7'){periodMode='next7';syncPeriodUi();applyCentralStatus();return}}
      ...
      var printBtn=e.target.closest('[data-enc-print]');if(printBtn){printCurrentOrders();return}
    ...
    fsTarget.addEventListener('change',function(e){
      if(e.target&&(e.target.id==='encDateFrom'||e.target.id==='encDateTo')){var from=dateInputFrom(),to=dateInputTo(); if(from&&to&&from.value&&to.value&&from.value>to.value){var tmp=from.value;from.value=to.value;to.value=tmp} periodMode=(from&&from.value)||(to&&to.value)?'range':'all'; syncPeriodUi(); applyCentralStatus(); scrollToActivePanel();}
    });
