(function(){
  ...
  // Central de Encomendas: v796 — calendário nativo clicável, sem busca e status clicável.
  ...
  function statusCountsForPeriod(){
    ...
    allOrderCards().forEach(function(card){var status=cardStatus(card);if(counts.hasOwnProperty(status)&&cardMatchesDate(card))counts[status]+=1});
    return counts;
  }
  function refreshSummary(){
    ...
    var values={total:cards.length,nova:counts.nova,period:activePeriod.length,producao:counts.producao,receivable:formatBrl(receiveTotal)};
  ...
  function metricKeyForView(){if(viewMode==='total')return 'total';if(viewMode==='period')return 'period';if(viewMode==='receivable')return 'receivable';if(activeStatus==='nova')return 'nova';if(activeStatus==='producao')return 'producao';return ''}
  function emptyCopy(){
  ...
  function applyCentralView(){
    ...
    var totalTab=q('[data-enc-top-total]',fsTarget);if(totalTab){var totalOn=metricActive==='total';totalTab.classList.toggle('active',totalOn);totalTab.classList.toggle('metric-active',totalOn);totalTab.setAttribute('aria-selected',totalOn?'true':'false')}
    qa('[data-enc-metric-card]',fsTarget).forEach(function(card){card.classList.toggle('active',card.dataset.encMetricCard===metricActive)});
    var visibleTotal=0;
    qa('[data-enc-status-panel]',fsTarget).forEach(function(panel){
      ...
      if(kind==='fluxo'){
        ...
        var copy=viewMode==='status'?(flowCopy[activeStatus]||['Acompanhamento','']):(viewMode==='total'?['Todas as encomendas','Todos os status e todas as datas']:(viewMode==='receivable'?['A receber','Encomendas com saldo pendente no período']:['Encomendas do período','Retiradas e entregas ativas no intervalo']));
  ...
  function printScopeLabel(scope){
    ...
    if(scope.indexOf('status:')===0){var key=scope.split(':')[1],labels={nova:'Novas',confirmada:'Confirmadas',producao:'Em produção',pronta:'Prontas',finalizada:'Finalizadas',cancelada:'Canceladas'};return period+' · '+(labels[key]||key)}
    return period;
  }
  function printOrders(scope){
    ...
    var w=window.open('','_blank','width=960,height=720');if(!w){toast('Não foi possível abrir a impressão. Verifique se o navegador bloqueou o pop-up.');return}try{w.document.open();w.document.write(html);w.document.close();try{w.opener=null}catch(e){}w.focus()}catch(e){try{w.close()}catch(_e){}toast('Não foi possível preparar a impressão.');}
  }
  function selectMetric(mode){
    if(mode==='total'){viewMode='total'}else if(mode==='period'){viewMode='period'}else if(mode==='receivable'){viewMode='receivable'}else{viewMode='status';activeStatus=mode||'nova'}
  ...
  if(fsTarget){
    ...
    fsTarget.addEventListener('click',function(e){
      ...
      var statusTrigger=e.target.closest('[data-enc-status-trigger]');if(statusTrigger){var wrap=statusTrigger.closest('[data-enc-status-quick]'),menu=wrap?q('[data-enc-status-menu]',wrap):null;if(menu){var willOpen=menu.hidden;closeStatusMenus(menu);menu.hidden=!willOpen;statusTrigger.setAttribute('aria-expanded',willOpen?'true':'false');if(willOpen){menu.classList.remove('enc-status-menu-up-v928');if(window.innerWidth<=720){var rect=statusTrigger.getBoundingClientRect(),menuHeight=Math.max(188,menu.scrollHeight||0),safeBottom=84;if(rect.bottom+menuHeight+10>window.innerHeight-safeBottom){menu.classList.add('enc-status-menu-up-v928')}}}else{menu.classList.remove('enc-status-menu-up-v928')}}return}
      ...
      var detail=e.target.closest('[data-enc-detail]');if(detail){openOrderDetailV849(detail);return}
    ...
    fsTarget.addEventListener('submit',function(e){var form=e.target;if(!form||!form.matches('form[action*="/encomendas/"][action$="/status"]'))return;e.preventDefault();if(form.dataset.encPaymentRequired==='1'){var card=form.closest('[data-enc-order]'),payButton=card?q('[data-enc-pay-open-v821]',card):null;if(payButton){payButton.click()}toast('Ainda há saldo pendente. Confirme o pagamento antes de finalizar.');return}submitStatusAjax(form)});
  }
