(function(){
  ...
  // V928 — confirmação visual do próprio MenuFacile (sem popup nativo do navegador).
  function confirmV928(message,{title='Confirmar exclusão',confirmText='Excluir'}={}){
    return new Promise(resolve=>{
      let modal=q('#encConfirmV928');
      if(!modal){
        modal=document.createElement('div');
        modal.id='encConfirmV928';
        modal.className='enc-confirm-v928';
        modal.hidden=true;
        modal.innerHTML=`<div class="enc-confirm-card-v928" role="dialog" aria-modal="true" aria-labelledby="encConfirmTitleV928"><div class="enc-confirm-icon-v928"><i class="fa-regular fa-trash-can"></i></div><h3 id="encConfirmTitleV928"></h3><p data-enc-confirm-message-v928></p><div class="enc-confirm-actions-v928"><button type="button" class="enc-confirm-cancel-v928">Cancelar</button><button type="button" class="enc-confirm-ok-v928"></button></div></div>`;
        document.body.appendChild(modal);
      }
      q('#encConfirmTitleV928',modal).textContent=title;
      q('[data-enc-confirm-message-v928]',modal).textContent=message;
      const cancel=q('.enc-confirm-cancel-v928',modal), ok=q('.enc-confirm-ok-v928',modal);
      ok.textContent=confirmText;
      modal.hidden=false;
      const oldOverflow=document.body.style.overflow; document.body.style.overflow='hidden';
      const finish=value=>{
        modal.hidden=true;document.body.style.overflow=oldOverflow;
        cancel.removeEventListener('click',onCancel);ok.removeEventListener('click',onOk);modal.removeEventListener('click',onBackdrop);document.removeEventListener('keydown',onKey);
        resolve(value);
      };
      const onCancel=()=>finish(false), onOk=()=>finish(true);
      const onBackdrop=e=>{if(e.target===modal)finish(false)};
      const onKey=e=>{if(e.key==='Escape')finish(false)};
      cancel.addEventListener('click',onCancel);ok.addEventListener('click',onOk);modal.addEventListener('click',onBackdrop);document.addEventListener('keydown',onKey);
  ...
  document.addEventListener('submit',async e=>{
    ...
    if(productForm){
      ...
      if(!(await confirmV928(`Tem certeza que deseja excluir o produto “${name}”? Ele deixará de aparecer no catálogo.`,{title:'Excluir produto'})))return;
    ...
    if(categoryForm){
      ...
      if(!(await confirmV928(`Tem certeza que deseja excluir a categoria “${name}”?`,{title:'Excluir categoria'})))return;
      if(btn)btn.disabled=true;
      try{
  ...
  // V938 — Sabores e Recheios para leigos: adicionar um por vez e excluir visualmente.
  // O hidden continua enviando CSV ao backend para manter 100% de compatibilidade.
  qa('[data-enc-list-editor-v938]',form).forEach(editor=>{
    const hidden=q('[data-enc-list-hidden-v938]',editor);
    const input=q('[data-enc-list-input-v938]',editor);
    const add=q('[data-enc-list-add-v938]',editor);
    const list=q('[data-enc-list-items-v938]',editor);
    let items=String(hidden?.value||'').split(',').map(x=>x.trim()).filter(Boolean);
    const dedupe=()=>{
      const seen=new Set();
      items=items.filter(value=>{const key=value.toLocaleLowerCase('pt-BR');if(seen.has(key))return false;seen.add(key);return true;});
    };
    const sync=()=>{if(hidden)hidden.value=items.join(', ')};
    const render=()=>{
      dedupe();sync();if(!list)return;
      if(!items.length){list.innerHTML='<div class="enc-list-empty-v938">Nenhum item adicionado.</div>';return;}
      list.innerHTML='';
      items.forEach((value,index)=>{
        const row=document.createElement('div');row.className='enc-list-row-v938';
        const name=document.createElement('span');name.textContent=value;
        const remove=document.createElement('button');remove.type='button';remove.className='enc-list-remove-v938';remove.setAttribute('aria-label','Excluir '+value);remove.title='Excluir';remove.innerHTML='<i class="fa-regular fa-trash-can"></i>';
    ...
    const addItem=()=>{
      ...
      if(items.some(x=>x.toLocaleLowerCase('pt-BR')===value.toLocaleLowerCase('pt-BR'))){
        showToast('Esse item já foi adicionado.','error');input?.select();return;
      }
      items.push(value);if(input)input.value='';render();input?.focus();
    };
    add?.addEventListener('click',addItem);
    input?.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();addItem();}});
    render();
  });
...
/* v813 — prazo flexível + Disponível hoje */
(function(){
  const check=document.querySelector('input[data-enc-hidden="encAvailableToday"]');
  const hidden=document.getElementById('encAvailableToday');
  const box=document.getElementById('encTodayBoxV813');
  const time=document.getElementById('encTodayTimeV813');
  if(!check||!hidden)return;
  const sync=()=>{
    hidden.value=check.checked?'1':'0';
    if(time)time.hidden=!check.checked;
    if(box)box.classList.toggle('is-on',check.checked);
  };
