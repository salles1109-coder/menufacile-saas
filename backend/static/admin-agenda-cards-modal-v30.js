(function(){
  'use strict';
  if(window.__MF_AGENDA_V38__)return;window.__MF_AGENDA_V38__=true;
  ...
  function statusKey(v){v=String(v||'pendente').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/\s+/g,'_');if(v==='aguardando_conclusao'||v==='awaiting_completion')return 'aguardando_conclusao';if(v==='concluida'||v==='concluido')return 'concluida';if(v==='paga'||v==='pago')return 'paga';if(v==='confirmado')return 'confirmada';if(v==='cancelado')return 'cancelada';if(v==='recusado')return 'recusada';if(v==='expirado')return 'expirada';return v}
  function statusLabel(v){var k=statusKey(v);return {pendente:'Pendente',confirmada:'Confirmado',aguardando_conclusao:'Aguardando conclusão',paga:'Pago',concluida:'Concluído',cancelada:'Cancelado',recusada:'Recusado',expirada:'Expirado'}[k]||'Pendente'}
  ...
  function choose(cards){var order={pendente:1,confirmada:2,aguardando_conclusao:3,paga:4,concluida:4,cancelada:5,recusada:6};return cards.slice().sort(function(a,b){return (order[statusKey(a.dataset.status)]||9)-(order[statusKey(b.dataset.status)]||9)})[0]}
  ...
  function decorate(){
    qa('#agendaGrid .slot').forEach(function(slot){
      slot.classList.remove('mf-v27-booked','mf-v27-pendente','mf-v27-confirmada','mf-v27-paga','mf-v27-concluida','mf-v27-cancelada','mf-v27-recusada','mf-v27-expirada','mf-v27-aguardando_conclusao');
