(function(){
  ...
  function sameLine(a,b){return Number(a.itemId)===Number(b.itemId)&&Number(a.optionId)===Number(b.optionId)&&String(a.fill||'')===String(b.fill||'')&&String(a.personalization||'')===String(b.personalization||'')&&String(a.date||'')===String(b.date||'')&&String(a.time||'')===String(b.time||'')}
  ...
  function leadMinutesForDate(dateValue,card=currentCard){
    const today=toDateInput(operationalNow());
    if(itemAvailableToday(card)&&dateValue===today)return itemTodayWaitMinutes(card);
  ...
  function ensureDate(){
    ...
    const todayEarliest=operationalNow();todayEarliest.setMinutes(todayEarliest.getMinutes()+itemTodayWaitMinutes(currentCard));
    const earliest=operationalNow();earliest.setMinutes(earliest.getMinutes()+itemAdvanceMinutes(currentCard));
    input.min=itemAvailableToday(currentCard)?toDateInput(todayEarliest):toDateInput(earliest);
  ...
  function renderCart(){
    ...
    box.innerHTML=cartState.map((item,index)=>{
      const details=[item.optionName,item.fill,item.personalization?('Personalização: '+item.personalization):''].filter(Boolean).join(' · ');
  ...
  async function jsonFetch(url,options){
    ...
    if(!res.ok||data.error||data.detail){
      const err=new Error(data.detail||data.error||'Não foi possível concluir esta etapa.');
      err.paymentDiagnostic=data.payment_debug_local?formatPaymentDiagnosticV784(data.diagnostico):'';
      throw err;
    }
    return data;
  }
  ...
  function paymentResultMarkupV786(kind,title,message,allowSwitch=true){
    return '<div class="enc-payment-result-v786 '+kind+'"><strong>'+escapeHtml(title)+'</strong><p>'+escapeHtml(message)+'</p>'+(allowSwitch?paymentSwitchButtonMarkupV786():'')+'</div>';
  }
  ...
  async function processPixOnline(order,email){
    ...
    if(status==='approved'){
      ...
      stage.innerHTML=paymentResultMarkupV786('is-approved','Pagamento aprovado ✓','O sinal foi confirmado e a encomenda já está registrada.',false);
    ...
    stage.innerHTML='<strong>Pix gerado</strong><p>Valor a pagar agora: <b>'+money(order.sinal_valor>0?order.sinal_valor:order.total)+'</b></p>'+qr+(code?'<div class="enc-copy-row"><input readonly value="'+escapeHtml(code)+'"><button type="button" data-copy-code>Copiar Pix</button></div>':'')+'<p>Assim que o Mercado Pago confirmar, o status da encomenda será atualizado automaticamente.</p>'+paymentSwitchButtonMarkupV786();
    onlinePaymentLocked=true;setSignalChoiceDisabledV783(true);
...
formData.payer={...(formData.payer||{}),first_name:additionalData.cardholderName};
}
      setSignalChoiceDisabledV783(true);q('#encCheckoutError').hidden=true;
      const result=await jsonFetch('/api/pagamentos/mercadopago/'+cfg.empresaId+'/encomenda/processar',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({encomenda_id:order.encomenda_id,public_token:order.public_token,email:email,formData:formData})});
      const status=String(result.status||'').toLowerCase();
      ...
      if(status==='approved'){
        stage.innerHTML=paymentResultMarkupV786('is-approved','Pagamento aprovado ✓','O sinal foi confirmado e a encomenda foi enviada ao estabelecimento.',false);
      ...
      resolve();
    }catch(err){setSignalChoiceDisabledV783(false);setError(err.message);reject(err)}})}});
  }
