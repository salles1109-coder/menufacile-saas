(function(){
  ...
  function toggleOrderDetails(button){
    var card=button&&button.closest?button.closest('[data-enc-order]'):null;if(!card)return;
    var opening=!card.classList.contains('enc-details-open-v942');
    card.classList.toggle('enc-details-open-v942',opening);
    button.setAttribute('aria-expanded',opening?'true':'false');
    var icon=q('i',button);if(icon)icon.className='fa-regular '+(opening?'fa-eye-slash':'fa-eye');
    var textNodes=Array.from(button.childNodes).filter(function(n){return n.nodeType===3&&String(n.nodeValue||'').trim()});
    if(textNodes.length)textNodes[textNodes.length-1].nodeValue=opening?' Ocultar detalhes':' Ver detalhes';
    button.setAttribute('aria-label',opening?'Ocultar detalhes':'Ver detalhes');
    button.setAttribute('title',opening?'Ocultar detalhes':'Ver detalhes');
    if(opening){setTimeout(function(){try{card.scrollIntoView({behavior:'smooth',block:'nearest'})}catch(e){}},20)}
  }
  ...
  if(fsTarget){
    ...
    fsTarget.addEventListener('click',function(e){
      ...
      var detail=e.target.closest('[data-enc-detail]');if(detail){e.preventDefault();e.stopPropagation();toggleOrderDetails(detail);return}
