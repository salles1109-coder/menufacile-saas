(function(){
  ...
  function allowedReceiving(){
    ...
    if(cartState?.allowPickup && menu.retirada!==false && del.retirada_ativa!==false)result.push({id:'retirada',icon:'fa-store',name:'Retirada',desc:''});
    if(cartState?.allowDelivery && menu.entrega!==false && del.entrega_ativa!==false)result.push({id:'entrega',icon:'fa-truck',name:'Entrega',desc:''});
    return result;
  }
  function signalMethod(){
    if(signalAmount(cartState?.price||0)<=0)return'';
    if(paymentCfg.pixManualChave)return'pix_sinal_manual';
    if(paymentCfg.onlineAtivo&&paymentCfg.pixOnline)return'pix_online';
    if(paymentCfg.onlineAtivo&&paymentCfg.cartaoOnline)return'cartao_online';
    return'';
  }
  function signalMethodLabel(method){
    if(method==='pix_sinal_manual'||method==='pix_online')return'Pix';
    if(method==='cartao_online')return'Cartão online';
    return'A definir';
  }
  function allowedPayments(){
    const p=paymentCfg,result=[];const signal=signalAmount(cartState?.price||0);const balance=Math.max(0,(cartState?.price||0)-signal);
    // Com sinal, a escolha abaixo representa como o cliente pretende pagar somente o saldo restante.
    if(signal>0){
      if(balance<=0)return result;
      if(p.cartaoPresencial)result.push({id:'cartao_presencial',icon:'fa-credit-card',name:'Cartão',desc:''});
      if(p.dinheiro)result.push({id:'dinheiro',icon:'fa-money-bill-wave',name:'Dinheiro',desc:''});
      return result;
    }
    if(p.cartaoPresencial)result.push({id:'cartao_presencial',icon:'fa-credit-card',name:'Cartão',desc:''});
    if(p.dinheiro)result.push({id:'dinheiro',icon:'fa-money-bill-wave',name:'Dinheiro',desc:''});
  ...
  function renderCheckout(){
    ...
    const signal=signalAmount(cartState.price),balance=Math.max(0,cartState.price-signal),payLabel=q('#encPaymentBalanceLabel');
    const payments=allowedPayments(),payBox=q('#encPaymentOptions');
    if(payLabel){payLabel.hidden=!(signal>0&&balance>0);payLabel.textContent='Como prefere pagar o saldo restante?'}
    if(signal>0&&balance<=0){payBox.innerHTML='';payBox.hidden=true}else{payBox.hidden=false;payBox.innerHTML=payments.length?payments.map((o,i)=>optionMarkup(o,'enc-payment',i)).join(''):'<div class="enc-checkout-error enc-inline-payment-error">Nenhuma forma de pagamento para o saldo está habilitada.</div>'}
  ...
  function payloadFromCheckout(){
    ...
    const signal=signalAmount(cartState?.price||0),balance=Math.max(0,(cartState?.price||0)-signal),sigMethod=signalMethod();
    let payment=q('input[name="enc-payment"]:checked')?.value||'';
    if(name.length<2)throw new Error('Informe seu nome.');if(phone.replace(/\D/g,'').length<8)throw new Error('Informe um WhatsApp válido.');if(!receive)throw new Error('Escolha retirada ou entrega.');
    if(signal>0&&!sigMethod)throw new Error('O estabelecimento exige sinal, mas ainda não configurou uma forma de recebê-lo.');
    if(balance>0&&!payment)throw new Error('Escolha como prefere pagar o saldo restante.');
