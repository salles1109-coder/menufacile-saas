(function(){
  ...
  // V958 — filtro inicia sempre vazio; Limpar só aparece após seleção real de data.
  ...
  function syncClearDateVisibilityV957(){
    if(!fsTarget)return;
    var inputFrom=dateInputFrom(),inputTo=dateInputTo();
    var hasDate=!!((inputFrom&&String(inputFrom.value||'').trim())||(inputTo&&String(inputTo.value||'').trim()));
    var rangeBox=q('.mf-v948-range',fsTarget);
    var slot=q('[data-enc-clear-slot-v957]',fsTarget);
    var clear=q('[data-enc-clear-dates-v948]',fsTarget);
    if(rangeBox)rangeBox.classList.toggle('mf-v956-has-date',hasDate);
    if(!hasDate){
    ...
    if(!clear&&slot){
      ...
      clear.className='mf-v948-clear-date';
      clear.type='button';
      clear.setAttribute('data-enc-clear-dates-v948','');
      clear.title='Remover filtro de datas';
      clear.innerHTML='<i class="fa-solid fa-xmark"></i><span>Limpar</span>';
      slot.replaceWith(clear);
    }
  }
  function resetDateFilterV958(){
    if(!fsTarget)return;
    rangeFrom='';
    rangeTo='';
    periodMode='none';
    ...
    if(clear){
      if(!slot){
        ...
        slot.className='mf-v957-clear-slot';
        slot.setAttribute('data-enc-clear-slot-v957','');
      }
      clear.replaceWith(slot);
    }
    var rangeBox=q('.mf-v948-range',fsTarget);
    if(rangeBox)rangeBox.classList.remove('mf-v956-has-date');
  }
  function syncPeriodUi(){
    ...
    var clear=q('[data-enc-clear-dates-v948]',fsTarget);if(clear)clear.hidden=periodMode==='none';syncClearDateVisibilityV957();
  }
  ...
  function printOrders(scope){
    ...
    try{
      ...
      w.focus();
      setTimeout(function(){
        try{if(!w.closed){w.focus();w.print()}}catch(err){console.error('[MenuFacile] Falha ao imprimir:',err)}
      },300);
    }catch(err){
      console.error('[MenuFacile] Falha ao preparar impressão:',err);
  ...
  if(fsTarget){
    readActiveStatus();activeStatus='nova';viewMode='status';resetDateFilterV958();syncPeriodUi();applyCentralView();syncFullscreenButton();
    window.addEventListener('pageshow',function(){resetDateFilterV958();syncPeriodUi();applyCentralView()});
    fsTarget.addEventListener('click',function(e){
      var clearDates=e.target.closest('[data-enc-clear-dates-v948]');if(clearDates){e.preventDefault();var slot=document.createElement('span');slot.className='mf-v957-clear-slot';slot.setAttribute('data-enc-clear-slot-v957','');clearDates.replaceWith(slot);rangeFrom='';rangeTo='';periodMode='none';syncPeriodUi();syncClearDateVisibilityV957();applyCentralView();return}
    ...
    fsTarget.addEventListener('change',function(e){if(e.target&&(e.target.id==='encDateFrom'||e.target.id==='encDateTo')){var from=dateInputFrom(),to=dateInputTo();rangeFrom=from?String(from.value||''):'';rangeTo=to?String(to.value||''):'';if(rangeFrom&&rangeTo&&rangeFrom>rangeTo){var tmp=rangeFrom;rangeFrom=rangeTo;rangeTo=tmp}periodMode=(!rangeFrom&&!rangeTo)?'none':'range';syncPeriodUi();syncClearDateVisibilityV957();applyCentralView();scrollToCurrentView()}});
    /* V955: não ativa o filtro apenas ao tocar/focar no campo.
       No celular, o seletor de data pode disparar focus antes de o usuário escolher uma data.
       O filtro e o botão Limpar só passam a existir depois do evento change com uma data real. */
